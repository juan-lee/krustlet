//! A custom kubelet backend that can run [Ignite](https://ignite.readthedocs.io/en/stable/) based workloads
//!
//! The crate provides the [`IgniteProvider`] type which can be used
...
//!     let provider = IgniteProvider::new(store, &kubelet_config, kubeconfig.clone()).await.unwrap();
//!     
...
use ignite_runtime::{HandleStopper, IgniteRuntime};
use kubelet::handle::{key_from_pod, pod_key, PodHandle};

const TARGET_IGNITE: &str = "ignite";
const LOG_DIR_NAME: &str = "wasi-logs";
const VOLUME_DIR: &str = "volumes";

/// IgniteProvider provides a Kubelet runtime implementation that executes WASM
/// binaries conforming to the WASI spec
#[derive(Clone)]
pub struct IgniteProvider<S> {
...
impl<S: ModuleStore + Send + Sync> IgniteProvider<S> {
...
impl<S: ModuleStore + Send + Sync> Provider for IgniteProvider<S> {
    const ARCH: &'static str = TARGET_IGNITE;
    ...
    async fn modify(&self, pod: Pod) -> anyhow::Result<()> {
        ...
        debug!("Modified pod spec: {:#?}", pod.name());
        if let Some(_timestamp) = pod.deletion_timestamp() {
            ...
            match handles.get_mut(&key_from_pod(&pod)) {
                Some(h) => {
                    ...
                    debug!("Deleting pod spec: {:#?}", pod.name());
